...
buttons_to_exchange.add(types.InlineKeyboardButton(text='Обменять криптовалюту', callback_data='change'),
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(f"Чтобы получить список команд введите команду /help", reply_markup=button)
...
async def handle_number(message: types.Message):
    ...
    buttons_ready_to_exchange.add(types.InlineKeyboardButton(text=f'Обменять {global_amount} BTC на USDT',
                                                             web_app=WebAppInfo(
                                                                 url=f'{get_payment_url_btc(global_amount)}')),
                                  types.InlineKeyboardButton(text=f'Обменять {global_amount} USDT на BTC',
                                                             web_app=WebAppInfo(
                                                                 url=f'{get_payment_url_usdt(global_amount)}')))
